...
class GuildsRepository
{
    ...
    public static function createGuildsByRestCordUser($data, $user_id)
    {
        foreach ($data as $key => $value) {
            ...
            if (empty(UserToGuilds::find()->where(['AND',['user_id' => $user_id],['guild_id' => $guild->id]])->all())) {
                $UTG = new UserToGuilds;
                $UTG->user_id = $user_id;
                $UTG->guild_id = $guild->id;
                $UTG->save();
            }
        }
    }

    public static function getUserAllGuildsByToken($access_key)
    {
        $discord = new DiscordClient([
            'token' => $access_key,
            'tokenType' => 'OAuth'
        ]);
        $user_guilds = $discord->user->getCurrentUserGuilds();
        GuildsRepository::createGuildsByRestCordUser($user_guilds, Yii::$app->user->id);
        return $user_guilds;
    }
}